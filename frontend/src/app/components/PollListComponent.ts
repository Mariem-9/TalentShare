    ...
    template: `
        <div style="padding: 1rem;">
            ...
            <ng-container *ngIf="polls.length; else noPolls">
                <p-carousel [value]="polls" [numVisible]="3" [numScroll]="1" [circular]="true" [autoplayInterval]="0" [showIndicators]="true" [style.max-width.px]="900" [style.margin]="'auto'">
                    <ng-template let-poll pTemplate="item">
                        <div class="poll-card" style="margin: 0 0.25rem;">
                            <app-poll-display [pollId]="poll.id" [pollType]="getPollType(poll)" [groupId]="groupId"> </app-poll-display>
                        </div>
                    </ng-template>
                </p-carousel>
            </ng-container>
